---
name: nlm-skill
description: "Expert guide for the NotebookLM CLI (`nlm`) and MCP server - interfaces for Google NotebookLM. Use this skill when users want to interact with NotebookLM programmatically, including: creating/managing notebooks, adding sources (URLs, YouTube, text, Google Drive), generating content (podcasts, reports, quizzes, flashcards, mind maps, slides, infographics, videos, data tables), conducting research, chatting with sources, or automating NotebookLM workflows. For executive/board Video Overview with KPI motion prompting, load references/executive-studio-video.md and any project-specific EXECUTIVE_VIDEO_PROMPT.md documented in AGENTS.md. Triggers on mentions of \"nlm\", \"notebooklm\", \"notebook lm\", \"podcast generation\", \"audio overview\", or any NotebookLM-related automation task."
version: "0.5.6"
---
...
12. **Source refresh hygiene (MCP/CLI):** After re-uploading the **same-titled** file(s) to an existing notebook, **list sources**, then **delete older duplicates** by title so only one current version remains. If the repository defines a notebook source hygiene workflow, follow it; otherwise apply deduplication by title after each `source_add`. `source_add` does **not** replace in place.
...
### Executive / director-facing Studio video (KPI + motion)

For **leadership Video Overview** (quantified ROI, brand palette, composited slide PNGs, **animation-friendly prompting**):

- Read **[references/executive-studio-video.md](references/executive-studio-video.md)** — MCP parameters, **Silent Bottleneck–style motion formula**, **presentation-native animated deck** (slides integrated into motion, not empty analogy frames), source hygiene; notebook IDs and prompt file paths come from your `AGENTS.md`, not this catalog.
- **Canonical copy-paste prompt** (scenes + `focus_prompt` fence): use the path documented in your root `AGENTS.md` for executive video prompts, or only `references/executive-studio-video.md` in this skill pack.
- **MCP**: `studio_create` with `artifact_type=video`, `confirm=true`, `focus_prompt` from that doc (include **CINEMATIC DELIVERY** at the top when leadership wants a fully film-like cut). Prefer `video_format=cinematic` in the **web UI**; on MCP, **`cinematic` may error** — use **`explainer`** with the same cinematic wording in `focus_prompt`. If the cut is flat, regenerate.
...
- **[references/executive-studio-video.md](references/executive-studio-video.md)**: Executive Video Overview + motion prompting (directors / board)
