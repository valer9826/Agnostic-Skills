...
## Workflow: Refresh local file sources without duplicates (MCP or CLI)

**Goal:** Updated `.md` / `.png` (or other allowed files) land in the notebook **once** per logical document—no stale duplicate sources.

**Normative reference:** Use your repository’s notebook source hygiene workflow if one exists (for example under `.agents/workflows/`); otherwise apply the MCP/CLI sequence below.

**MCP sequence (typical):**

1. `source_add` for each updated file (`file_path`, `wait=true` as needed).
2. `notebook_get` → inspect `sources[]` for **duplicate titles**.
3. For each duplicated title, `source_delete` with `source_ids=[...]` of the **old** IDs only, `confirm=True` (after user approval if policy requires).

**CLI sequence (typical):**

1. `nlm source add <notebook-id> --file /path/to/updated.md`
2. `nlm source list <notebook-id>` → identify duplicate titles.
3. `nlm source delete <old-source-id> --confirm` per stale row (with user confirmation per skill rules).

**Why:** `source_add` **appends** a new source; older uploads remain and can confuse answers and Studio artifacts.
